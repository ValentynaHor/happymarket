...
{ 
	if($output[$i]['hitsalesPrice'] == 0 OR $output[$i]['hitsalesPrice'] == '' OR $output[$i]['hitsalesImage'] == '') $style = 'style="background-color:#FFD5D5"'; else $style = '';
	...
	echo '<tr align="center">';
		...
		echo '<td class="row1" '.$style.'>';
			echo '<nobr>'.$output[$i]['hitsalesPrice'].' '.$CUR_CURRENCY_HIT[$output[$i]['hitsalesCurrency']].'</nobr>';
			if(!empty($KOEF_CURRENCY[$output[$i]['hitsalesBrand']]) AND $output[$i]['hitsalesCurrency'] != 'grn') echo '<br><nobr><span style="font-size:10px;">('.@round($output[$i]['hitsalesPrice']*$KOEF_CURRENCY_HIT[$output[$i]['hitsalesCurrency']],2).' грн.)</span></nobr>';
		echo '</td>';
		...
		echo '<td class="row1" style="padding:0px;">';
			...
			echo '<a href="/adm/?manageHits/del-hitsales/'.$output[$i]['hitsalesID'].'/';
...
<table width="90%">
  <tr>
	...
	<td align="left"> - не проставлена цена или фото (данный товар не будет отображаться на сайте).</td>
  </tr>
</table>
...
<?
	if ($outputCount[0]['count(permAll)'] > $countEntity)
	{
		...
		if($numPage != 1)
		{
			echo '<a href="/adm/?manageHits/page/'.($numPage-1).'" class="page" style="font-size:14px;" title="Предыдущая"><strong>&#160;&laquo;&#160;</strong></a>';
		}
		...
		{	
			...
			elseif($p == 1)
			{
				echo '<a href="/adm/?manageHits" class="page">&#160;'.$p.'&#160;</a>';
			}
			else
			{
				echo '<a href="/adm/?manageHits/page/'.$p.'" class="page">&#160;'.$p.'&#160;</a>';
			}
		}
		if($numPage != $countPages AND $viewModePage != 'all')
		{
			echo '<a href="/adm/?manageHits/page/'.($numPage+1).'" class="page" style="font-size:14px;" title="Следующая"><strong>&#160;&raquo;&#160;</strong></a>';
		}
		if($viewModePage == 'all')
		{
		...
		{
			echo '<a href="/adm/?manageHits/page/all" class="page" title="Показать все '.$categoryName.'">&#160; все</a>';
		}
		echo '</div>';
	}
?>